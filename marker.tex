...
It requires the storage of 4 points: two endpoints and two inner points.
...
Because it is based on points in a linear space, linear algebraic methods (matrix multiplications) can be used for calculating he perspective (?projective?) transformations.

In this project the second and the third options are used.
A quad can be initialised by it's size (the length of the base segment), the 
